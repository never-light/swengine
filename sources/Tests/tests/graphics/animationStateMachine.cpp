...
TEST_CASE("state-machine-states-clip-pose-node", "[animation]") {
    ...
    REQUIRE(MathUtils::isEqual(animationPose.getBoneLocalPose(0).getBoneMatrix(),
                               MathUtils::getTranslationMatix({ 15.0f, 0.0f, 0.0f })));
    ...
    glm::mat4 spinBoneTranslation = MathUtils::getTranslationMatix({ 0.0f, 15.0f, 0.0f });
...
TEST_CASE("state-machine-states-blend-pose-node", "[animation]") {
    ...
    SECTION("linear-blending") {
        SkeletalAnimationBlendPoseNode poseNode(firstAnimationClipInstance, secondAnimationClipInstance, blendFactorVariableId,
