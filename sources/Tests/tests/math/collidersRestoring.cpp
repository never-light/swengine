...
TEST_CASE("сolliders_parameters_restoring_AABB", "[math]")
{
  ...
  std::vector<glm::vec3>
    positions = MemoryUtils::createBinaryCompatibleVector<RawVector3, glm::vec3>(rawMesh.positions);
  AABB aabb = GeometryUtils::restoreAABBByVerticesList(positions);
...
TEST_CASE("сolliders_parameters_restoring_sphere", "[math]")
{
  ...
  std::vector<glm::vec3>
    positions = MemoryUtils::createBinaryCompatibleVector<RawVector3, glm::vec3>(rawMesh.positions);
  Sphere sphere = GeometryUtils::restoreSphereByVerticesList(positions);
