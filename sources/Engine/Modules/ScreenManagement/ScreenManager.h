...
class ScreenManager : public std::enable_shared_from_this<ScreenManager> {
 public:
  ...
  [[nodiscard]] std::shared_ptr<Screen> getScreen(const std::string& name) const;

  void changeScreen(const std::string& newScreenName);
  [[nodiscard]] std::shared_ptr<Screen> getActiveScreen() const;

  void update(float delta);
  void render();

  [[nodiscard]] std::shared_ptr<GUILayout> getCommonGUILayout() const;
