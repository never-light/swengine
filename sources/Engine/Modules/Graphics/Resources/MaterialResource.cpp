...
MaterialResource::ParametersType MaterialResource::buildDeclarationParameters(const pugi::xml_node& declarationNode,
  ...
  if (declarationNode.child("params")) {
    for (auto paramNode : declarationNode.child("params").children()) {
      ...
      switch (param.type) {
        case MaterialResourceParameters::ShaderParamType::Int:param.value = paramNode.attribute("value").as_int();
          break;

        case MaterialResourceParameters::ShaderParamType::Float:param.value = paramNode.attribute("value").as_float();
          break;
